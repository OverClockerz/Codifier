...
export const ConsolePanel: React.FC<Props> = ({
  ...
  // small CSS animation key for progress; injected inline because this repo does not have global css helper
  useEffect(() => {
    const id = 'progress-keyframes';
    if (!document.getElementById(id)) {
      const style = document.createElement('style');
      style.id = id;
      style.innerHTML = `@keyframes progress { 0% { transform: translateX(-25%); } 100% { transform: translateX(125%); } } .animate-[progress_2.2s_linear_infinite] { animation: progress 2.2s linear infinite; }`;
      ...
      <div className="flex-1 overflow-y-auto p-4 font-mono text-sm relative">
        {isLoading ? (
          <div className="absolute inset-0 z-10 bg-editor-bg/90 flex flex-col items-center justify-center text-slate-400 gap-4 backdrop-blur-sm">
            ...
            <div className="text-xs text-slate-500 italic">This may take a few seconds — the evaluator is performing a thorough analysis.</div>
          </div>
        ) : null}
